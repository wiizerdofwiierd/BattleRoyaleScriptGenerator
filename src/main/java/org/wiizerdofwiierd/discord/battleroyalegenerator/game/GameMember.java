...
public class GameMember{
	...
	/**
	 * Creates a new {@link GameMember} from a {@link Member} and generates default values
	 * @param member
	 */
	public GameMember(Member member){
		this(member.getIdLong(), Gender.MALE, member.getUser().getName(), member.getEffectiveName(), null, false, member.getUser().isBot(), false);
	}

	private GameMember(long id, Gender gender, String name, String nickname, String imgUrl, boolean participating, boolean bot, boolean custom){
		this.id = id;
		this.gender = gender;
		this.name = name;
	...
	public static GameMember createCustomMember(Gender gender, String name, String nickname, String imgUrl){
		return new GameMember(-1L, gender, name, nickname, imgUrl, false, false, true);
	}
